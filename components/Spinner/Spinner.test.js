...
describe('components', () => {
  describe('Spinner', () => {
    it('should mount with props', function () {
      const wrap = mount(<Spinner>Hello World</Spinner>)
    ...
    it('should render as type and with content', function () {
      const wrap = render(<Spinner>Hello World</Spinner>)

      expect(wrap[0].type).toEqual('tag')
      expect(wrap[0].name).toEqual('div')
      // contain styles
      expect(wrap[0].attribs.class).toContain('spinner')
