...
describe('components', () => {
  describe('GamePiece', () => {
    it('should mount with props', function () {
      const wrap = mount(<GamePiece>Hello World</GamePiece>)
    ...
    it('should render as type and with content', function () {
      const wrap = render(<GamePiece>Hello World</GamePiece>)

      expect(wrap[0].type).toEqual('tag')
      expect(wrap[0].name).toEqual('div')
      // contain styles
      expect(wrap[0].attribs.class).toContain('game_piece')
