...
describe('components', () => {
  describe('GameListPiece', () => {
    it('should mount with props', function () {
      const wrap = mount(<GameListPiece>Hello World</GameListPiece>)
    ...
    it('should render as type and with content', function () {
      const wrap = render(<GameListPiece>Hello World</GameListPiece>)

      expect(wrap[0].type).toEqual('tag')
      expect(wrap[0].name).toEqual('div')
      // contain styles
      expect(wrap[0].attribs.class).toContain('game_list_piece')
