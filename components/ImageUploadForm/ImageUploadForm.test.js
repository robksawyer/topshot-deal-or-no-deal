...
describe('components', () => {
  describe('ImageUploadForm', () => {
    it('should mount with props', function () {
      const wrap = mount(<ImageUploadForm>Hello World</ImageUploadForm>)
    ...
    it('should render as type and with content', function () {
      const wrap = render(<ImageUploadForm>Hello World</ImageUploadForm>)

      expect(wrap[0].type).toEqual('tag')
      expect(wrap[0].name).toEqual('div')
      // contain styles
      expect(wrap[0].attribs.class).toContain('image_upload_form')
